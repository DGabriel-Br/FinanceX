...
serve(async (req) => {
  ...
  try {
    ...
    // For trials, there might not be a payment intent yet
    let clientSecret = paymentIntent?.client_secret || null;
    
    // If no payment intent (trial), create a SetupIntent instead
    if (!clientSecret) {
      ...
      logStep("SetupIntent created for trial", { setupIntentId: setupIntent.id });
    }
